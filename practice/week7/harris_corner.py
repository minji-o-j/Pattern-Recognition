...
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) #1채널짜리 grayscale 이미지로 변환
...
res = np.copy(img)
threshold = 0.01 * conf_map.max()         # 임의의 임계값 (최댓값의 1%, flat 판별하는 곳)
res[conf_map > threshold] = [255, 0, 0]   # C가 일정 임계값보다 크면 corner, BLUE
res[conf_map <= threshold] = [0, 0, 255]  # C가 0에 가까우면 flat, RED
res[conf_map < 0] = [255, 255, 255]       # C가 음수이면 edge, WHITE

# 결과 시각화
concatenated = np.hstack((img, res)) #h: horizontal: 가로로 이미지 나열해줌
concatenated = cv2.resize(concatenated, None, fx=2.0, fy=2.0)  # 이미지 확대하기 위함. 중요하지 않음
cv2.imshow('harris corner', concatenated)
cv2.waitKey()
cv2.imwrite('result.png',concatenated)
